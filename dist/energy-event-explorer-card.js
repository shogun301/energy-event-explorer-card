...
function localDayRange(reference = new Date()) {
...
function shiftRangeByCalendarDays(start, end, days) {
...
function timeInWindows(timestamp, windows = []) {
...
function evaluateEnergyEvents({ series, rules, start, end, bucket_minutes = DEFAULT_BUCKET_MINUTES }) {
  validateRules(rules);
...
function integratePower(points, start, end, bucket_minutes = DEFAULT_BUCKET_MINUTES) {
...
class EnergyEventRangeSelectorCard extends HTMLElement {
  setConfig(config) {
...
class EnergySiteHistoryCard extends RangeAwareCard {
  setConfig(config) {
...
class EnergyEventExplorerCard extends RangeAwareCard {
  setConfig(config) {
  ...
  _renderShell() {
    this.shadowRoot.innerHTML = `<style>
      ...
      </style><ha-card><div class="head"><div class="titles"><div class="title">${escapeHtml(this.config.title)}</div><div class="subtitle">Aligned 5-minute observations · missing required metrics fail closed</div></div><button class="refresh" aria-label="Refresh events">↻</button></div><div class="body"><div class="status">Loading energy events…</div></div></ha-card>`;
    this.shadowRoot.querySelector(".refresh").addEventListener("click", () => this._load(true));
  }
  _render() {
...
window.customCards.push(
  ...
  { type: "energy-site-history-card", name: "Energy Site History", description: "Provider-neutral site and battery power history with integrated energy totals" },
  { type: "energy-event-explorer-card", name: "Energy Event Explorer", description: "Detect and time configurable energy events with fail-closed metric alignment" },
);
